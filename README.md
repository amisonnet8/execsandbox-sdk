...
Implemented in order, TinyGo (Go) then Rust — both have been published
to their language's package registry (see the badges below for current
versions). Each language gets its own top-level directory (`go/` for the
TinyGo version, `rust/` for the Rust version). See `PLAN.md` (Japanese)
for details.
...
published to pkg.go.dev and crates.io respectively (see the badges under
"Supported languages" above for current versions). See `PLAN.md`
