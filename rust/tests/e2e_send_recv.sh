#!/usr/bin/env bash
# execsandboxクレートのexamples/sender・examples/receiverが、SDKのsend/recv
# を介して実際のexecsandbox本体と正しく疎通することを確認するE2E。
#
# 本体のtests/e2e_basic.shと同じ構成・観測手段を踏襲する(go/tests/
# e2e_send_recv.shと同じ、言語がRustというだけの違い): senderは1回だけ
...
echo "tests/e2e_send_recv: building sender/receiver with cargo (wasm32-wasip1)..."
cargo_build_example "$REPO_ROOT/execsandbox" sender "$WORKDIR/sender.wasm"
cargo_build_example "$REPO_ROOT/execsandbox" receiver "$WORKDIR/receiver.wasm"
